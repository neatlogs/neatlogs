...
class SimulatedReranker:
    ...
    def rerank(self, documents: List[Dict], query: str) -> List[Dict]:
        ...
        """
        with neatlogs.trace("simulated_rerank", kind="RERANKER") as span:
            span.set_attribute("neatlogs.reranker.query", query)
            span.set_attribute("neatlogs.reranker.top_k", self.top_n)
            ...
            reranked = [item["doc"] for item in scored[:self.top_n]]
