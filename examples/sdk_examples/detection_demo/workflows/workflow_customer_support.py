...
# =============================================================================
@neatlogs.span(kind="AGENT", name="supervisor_agent")
def supervisor_agent(state: CustomerSupportState, llm: AzureChatOpenAI) -> dict:
    """
    ...
    system_tpl = neatlogs.PromptTemplate(
        "You are a customer support supervisor. Analyze the user's query and classify the intent:\n\n"
    ...
    user_tpl = neatlogs.UserPromptTemplate("User query: {{user_query}}")

    with neatlogs.trace("supervisor_classification", kind="LLM", prompt_template=system_tpl, user_prompt_template=user_tpl):
        system_msg = system_tpl.compile()
        user_msg = user_tpl.compile(user_query=user_query)
        response = llm.invoke([
...
def knowledge_agent(state: CustomerSupportState, llm: AzureChatOpenAI, retriever, reranker) -> dict:
    """
    ...
    system_tpl = neatlogs.PromptTemplate(
        "You are a helpful customer support agent. Use the provided documentation to answer the user's question concisely and accurately."
    )
    user_tpl = neatlogs.UserPromptTemplate("Documentation:\n{{context}}\n\nUser question: {{user_query}}")
    with neatlogs.trace("knowledge_agent_rag", kind="LLM", prompt_template=system_tpl, user_prompt_template=user_tpl):
        system_msg = system_tpl.compile()
        user_msg = user_tpl.compile(context=context, user_query=user_query)
        response = llm.invoke([
            SystemMessage(content=system_msg),
            HumanMessage(content=user_msg)
        ])
...
def orders_agent(state: CustomerSupportState, llm_with_tools: AzureChatOpenAI) -> dict:
    """
    ...
    system_tpl = neatlogs.PromptTemplate(
        "You are an order management agent. Use the available tools to help with order status, refunds, and tracking. Call the appropriate tool based on the user's request."
    )
    user_tpl = neatlogs.UserPromptTemplate("{{user_query}}")
    with neatlogs.trace("orders_agent_tool_calling", kind="LLM", prompt_template=system_tpl, user_prompt_template=user_tpl):
        system_msg = system_tpl.compile()
        user_msg = user_tpl.compile(user_query=user_query)
...
# =============================================================================
@neatlogs.span(kind="AGENT", name="handle_sensitive_request")
def handle_sensitive_request(state: CustomerSupportState, llm: AzureChatOpenAI) -> dict:
    """
    ...
    system_tpl = neatlogs.PromptTemplate(
        "You are a security-aware agent. The user is requesting sensitive data or admin access. "
        "Politely REFUSE this request and explain that you cannot provide such information for security reasons."
    )
    user_tpl = neatlogs.UserPromptTemplate("{{user_query}}")
    with neatlogs.trace("sensitive_request_refusal", kind="LLM", prompt_template=system_tpl, user_prompt_template=user_tpl):
        system_msg = system_tpl.compile()
        user_msg = user_tpl.compile(user_query=user_query)
        response = llm.invoke([
            SystemMessage(content=system_msg),
...
# Graph Builder
# =============================================================================
@neatlogs.span(kind="CHAIN", name="build_customer_support_graph")
