...
def generate_bid_recommendations(state: AdAutomationState) -> dict:
    ...
    with neatlogs.trace("ads_bid_reco_prompt", kind="LLM", prompt_template=prompt_template):
        prompt = prompt_template.compile(
            performance_json=json.dumps(state["current_performance"]),
            stock_json=json.dumps(state["stock_context"]),
            competitive_json=json.dumps(state["competitive_context"]),
            campaign_goal=state.get("campaign_goal", "maximize_roas"),
            budget_inr=f"₹{state.get('budget', 50_000):,}",
        )
...
def allocate_budget(state: AdAutomationState) -> dict:
    ...
    prompt_template = neatlogs.PromptTemplate(
        ...
        "Bid recommendations: {{bid_recommendations_json}}\n"
    ...
    with neatlogs.trace("ads_budget_allocator_prompt", kind="LLM", prompt_template=prompt_template):
        prompt = prompt_template.compile(
            bid_recommendations_json=json.dumps(state.get("bid_recommendations", [])),
            budget_inr=f"₹{state.get('budget', 50_000):,}",
            campaign_goal=state.get("campaign_goal", "maximize_roas"),
        )
        response = llm.invoke([HumanMessage(content=prompt)])
