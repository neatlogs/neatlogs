...
def classify_query(state: SupervisorState) -> dict:
    ...
    with neatlogs.trace("gobbs_router_prompt", kind="LLM", prompt_template=prompt_template):
...
def synthesize_response(state: SupervisorState) -> dict:
    ...
    with neatlogs.trace("gobbs_synth_prompt", kind="LLM", prompt_template=prompt_template):
        prompt = prompt_template.compile(
            user_query=state["user_query"],
            analysis_source=state.get("delegated_to", "Multi-agent"),
            analysis_results=json.dumps(state.get("sub_agent_result", {}), indent=2),
        )
        response = llm.invoke([HumanMessage(content=prompt)])
