"""
Stress Test 53: Multiple Workflow Calls WITHOUT Outer Wrapper
==============================================================

Tests whether calling the SAME workflow function multiple times (without outer wrapper)
creates SEPARATE trace_ids for each call.

Scenario:
- Call run_multi_agent_workflow("query 1")
- Call run_multi_agent_workflow("query 2")
- NO outer with trace() wrapper in main()
- Verify: Each call creates a DIFFERENT trace_id

Expected: 2 separate traces (trace_id_A for run 1, trace_id_B for run 2)

Run:
  python neatlogs/examples/53_multi_workflow_no_wrapper.py
"""
...
_env_default("NEATLOGS_LOG_SPANS_FILE", "spans_53_multi_workflow_no_wrapper.jsonl")
_env_default("NEATLOGS_LOG_RAW_SPANS_FILE", "spans_raw_53_multi_workflow_no_wrapper.jsonl")
_env_default("NEATLOGS_LOG_METRICS_FILE", "metrics_53_multi_workflow_no_wrapper.jsonl")
...
init(
    ...
    workflow_name="multi-workflow-no-wrapper",
...
def main():
    try:
        # Run workflow twice WITHOUT outer wrapper
        print("\n🔹 RUN 1: First workflow execution")
        query1 = "What are the key components of modern AI agent systems?"
        result1 = run_multi_agent_workflow(query1)
        print(f"\n📝 Answer 1:\n{result1}\n")

        print("\n🔹 RUN 2: Second workflow execution")
        ...
        shutdown()
        print("\n✅ Check output files:")
        print(f"   - spans_53_multi_workflow_no_wrapper.jsonl")
        print(f"   - spans_raw_53_multi_workflow_no_wrapper.jsonl")
        print(f"   - metrics_53_multi_workflow_no_wrapper.jsonl")
        print("\nVerify: Should have 2 DIFFERENT trace_ids (one per run)")
        print("Run: jq '.trace_id' spans_53_multi_workflow_no_wrapper.jsonl | sort | uniq -c")
