"""
Stress Test 54: Multiple Workflow Calls WITH Outer Wrapper
===========================================================

Tests whether calling the SAME workflow function multiple times WITHIN
an outer with trace() wrapper creates ONE shared trace_id.

Scenario:
- Outer: with trace("batch_session", kind="WORKFLOW")
- Inside: run_multi_agent_workflow("query 1")
- Inside: run_multi_agent_workflow("query 2")
- Verify: Both calls share the SAME trace_id

Expected: 1 trace containing spans from both workflow runs

Run:
  python neatlogs/examples/54_multi_workflow_with_wrapper.py
"""
...
_env_default("NEATLOGS_LOG_SPANS_FILE", "spans_54_multi_workflow_with_wrapper.jsonl")
_env_default("NEATLOGS_LOG_RAW_SPANS_FILE", "spans_raw_54_multi_workflow_with_wrapper.jsonl")
_env_default("NEATLOGS_LOG_METRICS_FILE", "metrics_54_multi_workflow_with_wrapper.jsonl")
...
init(
    ...
    workflow_name="multi-workflow-with-wrapper",
...
def main():
    try:
        # Outer wrapper - both workflow runs should share THIS trace context
        with trace("batch_session", kind="WORKFLOW"):
            print("\n🔹 RUN 1: First workflow execution (inside outer wrapper)")
            query1 = "What are the key components of modern AI agent systems?"
            result1 = run_multi_agent_workflow(query1)
            print(f"\n📝 Answer 1:\n{result1}\n")

            print("\n🔹 RUN 2: Second workflow execution (inside same outer wrapper)")
        ...
        shutdown()
        print("\n✅ Check output files:")
        print(f"   - spans_54_multi_workflow_with_wrapper.jsonl")
        print(f"   - spans_raw_54_multi_workflow_with_wrapper.jsonl")
        print(f"   - metrics_54_multi_workflow_with_wrapper.jsonl")
        print("\nVerify: Should have 1 SINGLE trace_id (both runs share it)")
        print("Run: jq '.trace_id' spans_54_multi_workflow_with_wrapper.jsonl | sort | uniq -c")
