"""
Stress Test 56: Same Trace Name in SEPARATE with trace() Blocks
================================================================

Tests whether using the SAME trace name in SEPARATE (not nested) with trace()
blocks creates separate or shared trace_ids.

Scenario:
- Block 1: with trace("batch_processing_session", kind="WORKFLOW"):
            workflow_a("query 1")
- Block 2: with trace("batch_processing_session", kind="WORKFLOW"):
            workflow_a("query 2")
- Verify: What happens when trace NAME is same but blocks are separate?

Expected: Document actual behavior (likely creates 2 separate trace_ids)

Run:
  python neatlogs/examples/56_same_trace_name_separate_blocks.py
"""
...
_env_default("NEATLOGS_LOG_SPANS_FILE", "spans_56_same_trace_name_separate_blocks.jsonl")
_env_default("NEATLOGS_LOG_RAW_SPANS_FILE", "spans_raw_56_same_trace_name_separate_blocks.jsonl")
_env_default("NEATLOGS_LOG_METRICS_FILE", "metrics_56_same_trace_name_separate_blocks.jsonl")
...
init(
    ...
    workflow_name="same-trace-name-separate-blocks",
...
def main():
    try:
        # SEPARATE with trace() blocks with SAME NAME
        print("\n🔹 BLOCK 1: First with trace() block")
        with trace("batch_processing_session", kind="WORKFLOW"):
            query1 = "What are the key components of modern AI agent systems?"
            result1 = run_multi_agent_workflow(query1)
            print(f"\n📝 Answer 1:\n{result1}\n")

        print("\n🔹 BLOCK 2: Second with trace() block (SAME NAME)")
        with trace("batch_processing_session", kind="WORKFLOW"):
            query2 = "How do multi-agent systems handle task decomposition?"
            result2 = run_multi_agent_workflow(query2)
            print(f"\n📝 Answer 2:\n{result2}\n")
        ...
        shutdown()
        print("\n✅ Check output files:")
        print(f"   - spans_56_same_trace_name_separate_blocks.jsonl")
        print(f"   - spans_raw_56_same_trace_name_separate_blocks.jsonl")
        print(f"   - metrics_56_same_trace_name_separate_blocks.jsonl")
        print("\nVerify: Document actual behavior (likely 2 different trace_ids)")
        print("Run: jq '.trace_id' spans_56_same_trace_name_separate_blocks.jsonl | sort | uniq -c")
