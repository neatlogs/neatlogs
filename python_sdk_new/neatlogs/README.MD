...
init(
    ...
    workflow_name="my-workflow",               # Optional: workflow label
    tags=["production", "api-v2"],             # Optional: global tags
)
```

**Supported instrumentations**: `openai`, `anthropic`, `langchain`, `llamaindex`, `crewai`, `haystack`, `google_genai`, `mcp`
...
def main():
    with trace(name="batch_processor"):
        workflow_1()
        workflow_2()

# Use case 3: Multi-turn session tracking
init(api_key="...", session_id="user-456")
with trace(name="conversation_turn_1"):
    agent_workflow("Hi")
with trace(name="conversation_turn_2"):
    agent_workflow("Tell me more")
```
...
# Group conversation turns under the SDK session
with trace(name="chat_session"):
    history = []
...
4. **Session grouping via `init()`** - Set `session_id` (or `auto_session=True`) and use `trace(name=...)` for per-turn grouping
...
See `/neatlogs/examples` for 60+ examples. For UI demos with varied trace shapes, start with:

- `neatlogs/examples/00_simplified_api.py` (clean baseline: WORKFLOW/AGENT/LLM)
- `neatlogs/examples/47_all_span_kinds.py` (full span-kind matrix)
- `neatlogs/examples/48_multi_workflow.py` (cross-workflow traces)
- `neatlogs/examples/45_streaming_matrix.py` (streaming vs non-streaming)
- `neatlogs/examples/43_agentic_systems_tooling.py` (agent + tool + MCP boundaries)
- `neatlogs/examples/27_mcp_client.py` + `neatlogs/examples/shared_mcp_server.py` (MCP end-to-end)
- `neatlogs/examples/61_langchain_openai_chroma.py` (practical RAG)
- `neatlogs/examples/69_trace_burst_load.py` (burst/load traces)
