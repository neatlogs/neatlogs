...
Agents:
  - openai_reasoning_agent    — OpenAI o4-mini, non-streaming, reasoning_effort + max_completion_tokens
  - openai_full_params_agent  — OpenAI gpt-4o, streaming, all 6 standard params
  - anthropic_thinking_agent  — claude-sonnet-4-6, streaming, extended thinking
  - langchain_openai_agent    — LangChain ChatOpenAI (gpt-4o-mini), temperature + max_tokens
...
def openai_reasoning_agent(problem: str) -> str:
    with neatlogs.trace("o4_mini_reasoning", kind="LLM", prompt_template=_reasoning_sys,
        ...
        response = openai_client.chat.completions.create(
            ...
            reasoning_effort="high",        # → llm.invocation_parameters
...
def openai_full_params_agent(problem: str) -> str:
    with neatlogs.trace("gpt4o_full_params", kind="LLM", prompt_template=_reasoning_sys,
        ...
        stream = openai_client.chat.completions.create(
            ...
            presence_penalty=0.1,         # → llm.invocation_parameters
            frequency_penalty=0.1,        # → llm.invocation_parameters
            seed=42,                      # → llm.invocation_parameters
            max_tokens=1000,              # → llm.invocation_parameters
            stream=True,
        )
        print(f"\n--- {LLM_MODEL} (streaming) ---")
        full = ""
...
def langchain_openai_agent(problem: str) -> str:
    llm = ChatOpenAI(
        model="gpt-4o-mini",
        temperature=0.5,    # → llm.invocation_parameters
        max_tokens=500,     # → llm.invocation_parameters
        top_p=0.95,         # → llm.invocation_parameters
    )
    with neatlogs.trace("langchain_gpt4o_mini", kind="LLM", prompt_template=_reasoning_sys,
                        user_prompt_template=_reasoning_user):
