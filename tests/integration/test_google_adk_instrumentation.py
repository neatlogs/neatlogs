...
class TestGoogleGenAIInstrumentation:
    ...
    def test_gemini_generate_content(self):
        ...
        # ACT
        response = client.models.generate_content(model="gemini-2.0-flash-exp", contents="Say hi")
    ...
    def test_gemini_streaming(self):
        ...
        mock_content = [json.dumps(chunk_1).encode("utf-8"), json.dumps(chunk_2).encode("utf-8")]
        ...
        for chunk in client.models.generate_content_stream(
            model="gemini-1.5-pro", contents="Stream me"
        ):
    ...
    async def test_gemini_async(self):
        ...
        respx.post(url__regex=r"https://generativelanguage.googleapis.com.*").mock(
            return_value=httpx.Response(
                200, json={"candidates": [{"content": {"parts": [{"text": "Async Hi"}]}}]}
            )
        )
        ...
        response = await client.aio.models.generate_content(
            model="gemini-1.5-flash", contents="Async test"
        )
