...
try:
    from agno import Agent, RunConfig, KnowledgeBase, Tool
    from agno.models import OpenAIChat, Anthropic, GoogleGenerativeAI, Groq
...
class TestAgnoInstrumentation:
    ...
    def test_basic_agent_sync(self, mock_openai_chat_response):
        ...
        llm_spans = [s for s in spans if any(
            x in s.name for x in ['OpenAI', 'Chat', 'LLM'])]
        assert len(
            llm_spans) >= 1, f"No LLM span found. Spans: {[s.name for s in spans]}"
    ...
    def test_multi_model_agents(self, mock_openai_chat_response, mock_anthropic_response):
        ...
        openai_spans = [
            s for s in spans if "openai" in s.name.lower() or "gpt" in s.name.lower()]
        anthropic_spans = [
    ...
    def test_tic_tac_toe_simulation(self, mock_openai_chat_response, mock_openai_o3_response):
        ...
        llm_spans = [s for s in spans if any(
            x in s.name.lower() for x in ['openai', 'chat', 'llm'])]
    ...
    def test_agent_turn_based_game(self, mock_google_gemini_response, mock_groq_llama_response):
        ...
        groq_spans = [s for s in spans if "groq" in s.name.lower()
                      or "llama" in s.name.lower()]
        ...
        for span in groq_spans:
            assert "groq" in span.attributes.get("llm.system", "").lower() or \
                   "llama" in span.attributes.get("llm.model_name", "").lower()
    ...
    def test_agent_with_builtin_tools(self, mock_openai_chat_response):
        ...
        assert len(
            tool_spans) >= 1, f"Tool span missing. Found: {[s.name for s in spans]}"
    ...
    def test_agent_with_custom_toolkit(self, mock_openai_chat_response):
        ...
        tool_span = tool_spans[0]
        assert "metrics" in str(tool_span.attributes).lower() or \
               "data" in str(tool_span.attributes).lower()
    ...
    @respx.mock
    def test_model_routing_agent(self, mock_openai_chat_response, mock_anthropic_response, mock_google_gemini_response):
        """
        ...
        respx.post("https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent").mock(
            return_value=httpx.Response(200, json=mock_google_gemini_response)
        )
        ...
        openai_spans = [
            s for s in spans if "openai" in str(s.attributes).lower()]
        anthropic_spans = [
            s for s in spans if "anthropic" in str(s.attributes).lower()]
        google_spans = [
            s for s in spans if "google" in str(s.attributes).lower()]
    ...
    def test_agent_streaming(self, mock_openai_chat_response):
        ...
        if hasattr(llm_span, 'events'):
            stream_events = [e for e in llm_span.events if "chunk" in e.name.lower(
            ) or "stream" in e.name.lower()]
            if stream_events:
    ...
    def test_agent_with_knowledge_base(self, mock_openai_chat_response, mock_embeddings_response):
        ...
        with patch.object(kb, 'search', return_value=["Document 1: Test content", "Document 2: More test"]):
            agent = Agent(
                name="RAGAgent",
                model=OpenAIChat(id="gpt-4o"),
                knowledge_base=kb,
        ...
        kb_spans = [s for s in spans if "knowledge" in s.name.lower()
                    or "KnowledgeBase" in s.name]
        if kb_spans:
        ...
        if agent_spans:
            ...
            assert "knowledge" in str(agent_span.attributes).lower() or \
                   "rag" in str(agent_span.attributes).lower()
    ...
    def test_multi_knowledge_base_agent(self, mock_openai_chat_response, mock_embeddings_response):
        ...
        tech_kb = KnowledgeBase(
            name="TechDocs", description="Technical documentation")
        legal_kb = KnowledgeBase(
            name="LegalDocs", description="Legal documents")

        # Mock search methods
        with patch.object(tech_kb, 'search', return_value=["API documentation", "Code examples"]):
            with patch.object(legal_kb, 'search', return_value=["Terms of service", "Privacy policy"]):
                ...
                response = agent.run(
                    "Explain API terms and technical implementation")

        spans = self.wait_for_spans(min_spans=5, timeout=2.0)

        # Check for multiple KB spans
        kb_spans = [s for s in spans if any(
            kb in s.name for kb in ["TechDocs", "LegalDocs", "Knowledge"])]
    ...
    def test_agent_workflow_orchestration(self, mock_openai_chat_response):
        ...
        agent_spans = [s for s in spans if "Agent" in s.name]
        agent_roles = [s.attributes.get(
            "agent.role") for s in agent_spans if s.attributes.get("agent.role")]
    ...
    def test_conditional_agent_execution(self, mock_openai_chat_response):
        ...
        decider_spans = [s for s in spans if s.attributes.get(
            "agent.name") == "DecisionAgent"]
        assert len(decider_spans) >= 2

        # Check both action agents were used (depends on decisions)
        action_a_spans = [s for s in spans if s.attributes.get(
            "agent.name") == "ActionAgentA"]
        action_b_spans = [s for s in spans if s.attributes.get(
            "agent.name") == "ActionAgentB"]
    ...
    def test_agent_error_handling(self, mock_openai_chat_response):
        ...
        assert "error" in str(error_span.status.description).lower() or \
               "500" in str(error_span.status.description) or \
               "failed" in str(error_span.status.description).lower()
    ...
    def test_agent_with_system_prompt_override(self, mock_openai_chat_response):
        ...
        if llm_spans:
            ...
            if "system" in str(llm_span.attributes).lower() or \
               "prompt" in str(llm_span.attributes).lower():
                assert "custom" in str(llm_span.attributes).lower() or \
                       "json" in str(llm_span.attributes).lower() or \
                       "confidence" in str(llm_span.attributes).lower()
