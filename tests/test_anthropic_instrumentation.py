...
class TestGoogleGenAIInstrumentation:
    ...
    def test_gemini_api_mode_creates_span(self, gemini_response_json, in_memory_span_exporter):
        ...
        assert span.attributes.get("llm.system") in [
            "google", "gemini", "google_genai"]

    @respx.mock
    def test_gemini_vertex_mode_creates_span(self, gemini_response_json, in_memory_span_exporter, mock_google_auth):
    ...
    def test_gemini_multipart_content_list(self, gemini_response_json, in_memory_span_exporter):
        ...
        if len(spans) == 0:
            pytest.fail(
                "CRITICAL: Gemini failed to track Multi-part (List) content.")

        # Verify input was flattened
        input_val = spans[0].attributes.get("input.value")
        assert "Who are you?" in str(
            input_val), "Input list was not captured correctly"
    ...
    def test_gemini_tool_calling(self, in_memory_span_exporter):
        ...
        has_tool = "get_weather" in str(span.attributes.get("llm.output_messages")) or \
                   "get_weather" in str(
                       span.attributes.get("llm.request.functions"))
