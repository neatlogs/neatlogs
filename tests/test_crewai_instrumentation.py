...
def create_mock_anthropic_response(content=None):
    ...
    return {
        ...
        "content": [{
            "type": "text",
            "text": content or "This is a test response."
        }],
...
class TestCrewAIInstrumentation:
    ...
    def test_basic_crew_sync(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Research completed on AI trends.")
        ...
        crew_spans = [s for s in spans if "crew" in s.name.lower()
                      or "crewai" in s.name.lower()]
        assert len(crew_spans) > 0 or len(
            spans) > 0, "Crew execution should create spans"
    ...
    async def test_basic_crew_async(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Async research completed.")
        ...
        researcher = Agent(
            role="Researcher",
            goal="Research async",
            backstory="Async researcher",
            verbose=True
        )
        task = Task(
            description="Research async patterns",
            agent=researcher,
            expected_output="Async summary"
        )
        crew = Crew(agents=[researcher], tasks=[task])
    ...
    def test_multi_agent_sequential(self, in_memory_span_exporter):
        ...
        agent_names = [s.attributes.get(
            "crewai.agent.role") for s in spans if s.attributes.get("crewai.agent.role")]
        if agent_names:
            assert "Researcher" in agent_names or "Writer" in agent_names, "Should track agent roles"
    ...
    def test_task_dependencies_context_flow(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Research summary: AI is advancing.")
    ...
    def test_parallel_tasks(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Parallel task completed.")

        # Mock multiple parallel calls
        respx.post(url__regex=r"https://api.openai.com/v1/chat/completions").mock(
            side_effect=[
                httpx.Response(200, json=create_mock_openai_response(
                    content="Task 1 done")),
                httpx.Response(200, json=create_mock_openai_response(
                    content="Task 2 done"))
            ]
        ...
        task1 = Task(
            ...
            agent=researcher,
            expected_output="Summary A"
        )
        task2 = Task(
            description="Research topic B",
            agent=researcher,
            expected_output="Summary B"
        )
    ...
    def test_multi_llm_agents(self, in_memory_span_exporter):
        ...
        respx.post(url__regex=r"https://api.openai.com/v1/chat/completions").mock(
            return_value=httpx.Response(200, json=create_mock_openai_response(
                content="OpenAI response", model="gpt-4o-mini"
            ))
        )

        # Mock Anthropic responses
        respx.post(url__regex=r"https://api.anthropic.com/v1/messages").mock(
            return_value=httpx.Response(200, json=create_mock_anthropic_response(
    ...
    def test_crew_with_tools(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Used tool to get weather.")
        ...
        tool_spans = [s for s in spans if "tool" in s.name.lower()
                      or "get_weather" in s.name.lower()]
    ...
    def test_nested_crews(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Nested crew task completed.")
        ...
        inner_agent = Agent(
            role="Inner Agent",
            goal="Inner task",
            backstory="Inner agent",
            verbose=True
        )
        inner_task = Task(
            description="Inner task: {topic}",
            agent=inner_agent,
            expected_output="Inner result"
        )
    ...
    def test_crew_with_memory(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Remembered context from previous run.")
        ...
        memory_spans = [s for s in spans if s.attributes.get("crewai.memory") or
                        "memory" in str(s.attributes).lower()]
    ...
    def test_agent_delegation(self, in_memory_span_exporter):
        ...
        delegation_spans = [
            s for s in spans if "delegat" in str(s.attributes).lower()]
    ...
    def test_complex_production_workflow(self, in_memory_span_exporter):
        ...
        respx.post(url__regex=r"https://api.openai.com/v1/chat/completions").mock(
            side_effect=[
                ...
                httpx.Response(200, json=create_mock_openai_response(
                    content="Analysis: Growing market")),
                httpx.Response(200, json=create_mock_openai_response(
                    content="Report: Comprehensive analysis"))
            ]
        )
        ...
        agent_roles = [s.attributes.get("crewai.agent.role") for s in spans
                       if s.attributes.get("crewai.agent.role")]
        if agent_roles:
            assert len(set(agent_roles)
                       ) >= 1, "Should track different agent roles"
    ...
    def test_crew_streaming(self, in_memory_span_exporter):
        ...
        respx.post(url__regex=r"https://api.openai.com/v1/chat/completions").mock(
            return_value=httpx.Response(200, content=stream_response(), headers={
                                        "content-type": "text/event-stream"})
        )

        researcher = Agent(
            role="Researcher",
            goal="Research with streaming",
            backstory="Researcher",
            verbose=True
        )
        ...
        task = Task(
            description="Research {topic}",
            agent=researcher,
            expected_output="Streaming research"
        )
    ...
    def test_custom_llm_configuration(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Custom LLM response.", model="gpt-4")
        ...
        models = [s.attributes.get("gen_ai.request.model") for s in spans
                  if s.attributes.get("gen_ai.request.model")]
        if models:
            assert any("gpt-4" in str(m)
                       for m in models), "Should track custom model"
    ...
    def test_input_variable_capture(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Research on Quantum Computing completed.")
    ...
    def test_crew_process_configuration(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Process configured task completed.")
    ...
    def test_crew_max_iterations_retries(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Task completed with retries.")
    ...
    def test_agent_template_variables(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Template-based agent task completed.")
    ...
    def test_crew_shared_context(self, in_memory_span_exporter):
        ...
        mock_resp = create_mock_openai_response(
            content="Shared context task completed.")
