...
class TestDSPyInstrumentation:
    ...
    def mock_openai_chat_response(self):
        ...
        return {
            ...
            "choices": [{
                ...
                "message": {
                    "role": "assistant",
                    "content": "This is a DSPy test response."
                },
                "finish_reason": "stop"
            }],
            "usage": {
    ...
    def assert_span_exists(self, span_name: str, spans: List = None):
        ...
        assert len(
            matching_spans) > 0, f"Span '{span_name}' not found in {[s.name for s in spans]}"
    ...
    def test_react_module(self, mock_openai_chat_response):
        ...
        tool_responses = [
            {"name": "search", "arguments": {
                "query": "weather in Paris"}, "result": "Sunny, 22°C"},
        ...
        with patch.object(react, 'tools', new_callable=MagicMock) as mock_tools:
            ...
            # Execute
            prediction = react(
                question="What's the weather in Paris and what's 2+2?")
        ...
        assert len(
            tool_spans) >= 1, f"Expected tool spans, found: {[s.name for s in spans]}"

        # Verify reasoning trace
        reasoning_spans = [
            s for s in spans if "reasoning" in str(s.attributes).lower()]
        if reasoning_spans:
    ...
    # =================================================================
    @respx.mock
    def test_rag_module(self, mock_openai_chat_response, mock_openai_embedding_response, mock_retrieval_results):
        """
        ...
        respx.post("https://api.openai.com/v1/embeddings").mock(
            return_value=httpx.Response(
                200, json=mock_openai_embedding_response)
        )
    ...
    @respx.mock
    def test_multi_hop_rag(self, mock_openai_chat_response, mock_openai_embedding_response, mock_retrieval_results):
        """
        ...
        class MultiHopRAG(Module):
            ...
            def forward(self, question):
                ...
                query2 = self.generate_query(
                    context=context1, question=question).query
                ...
                return dspy.Prediction(
                    ...
                    intermediate_query=query2
                )

        multi_hop_rag = MultiHopRAG()

        # Execute
        prediction = multi_hop_rag(
            question="Who founded the company that makes DSPy?")
        ...
        assert len(
            retrieve_spans) >= 2, f"Expected at least 2 Retrieve spans, got {len(retrieve_spans)}"

        # Check for query generation span
        query_spans = [s for s in spans if "Predict" in s.name and "query" in str(
            s.attributes).lower()]
    ...
    def test_program_compilation(self, mock_openai_chat_response):
        ...
        trainset = [
            dspy.Example(question="What is Python?",
                         answer="Programming language").with_inputs("question"),
            dspy.Example(question="Who created Linux?",
                         answer="Linus Torvalds").with_inputs("question")
        ]

        # Configure optimizer
        from dspy.teleprompt import BootstrapFewShot
        teleprompter = BootstrapFewShot(
            metric=lambda a, b: a.answer == b.answer)
        ...
        compilation_spans = [s for s in spans if "compilation" in s.name.lower(
        ) or "teleprompt" in s.name.lower()]
        assert len(compilation_spans) >= 1, "Missing compilation spans"

        # Check for optimization phases
        optimization_spans = [
            s for s in spans if "optimization" in str(s.attributes).lower()]
        ...
        compiled_spans = [
            s for s in predict_spans if "compiled" in str(s.attributes).lower()]
    ...
    def test_multi_chain_comparison(self, mock_openai_chat_response):
        ...
        respx.post("https://api.openai.com/v1/chat/completions").mock(
            side_effect=[httpx.Response(
                200, json=mock_openai_chat_response) for _ in range(5)]
        )
        ...
        chain_spans = [
            s for s in spans if "Chain" in s.name or "Predict" in s.name]
        assert len(
            chain_spans) >= 3, f"Expected at least 3 chain spans, got {len(chain_spans)}"

        # Check for comparison/voting span
        comparison_spans = [s for s in spans if "comparison" in s.name.lower(
        ) or "vote" in str(s.attributes).lower()]
    ...
    def test_custom_signature_module(self, mock_openai_chat_response):
        ...
        class WeatherAnalysis(Signature):
            ...
            weather_summary = OutputField(
                desc="Brief summary of weather conditions")
            recommendation = OutputField(
                desc="Recommendation for the activity")
            confidence = OutputField(
                desc="Confidence level 0-1", prefix="Confidence:")
        ...
        cot_span = self.assert_span_exists("ChainOfThought", spans)
        assert "WeatherAnalysis" in cot_span.attributes.get(
            "signature.name", "")
        ...
        output_attrs = [k for k in cot_span.attributes.keys()
                        if k.startswith("llm.output")]
    ...
    def test_multi_model_program(self, mock_openai_chat_response):
        ...
        respx.post("https://api.openai.com/v1/chat/completions").mock(
            side_effect=[
                httpx.Response(
                    200, json={**mock_openai_chat_response, "model": "gpt-4o"}),
                httpx.Response(
                    200, json={**mock_openai_chat_response, "model": "gpt-3.5-turbo"})
            ]
        )
        ...
        gpt4_spans = [
            s for s in spans if "gpt-4o" in str(s.attributes).lower()]
        gpt35_spans = [s for s in spans if "gpt-3.5" in str(
            s.attributes).lower() or "gpt-3.5-turbo" in str(s.attributes).lower()]
        ...
        for span in spans:
            if "ChainOfThought" in span.name:
                assert "gpt-4o" in span.attributes.get(
                    "llm.model.name", "").lower()
            if "Predict" in span.name and "QuickAnalysis" in span.attributes.get("signature.name", ""):
                assert "gpt-3.5" in span.attributes.get(
                    "llm.model.name", "").lower()
    ...
    def test_evaluation_metrics(self, mock_openai_chat_response):
        ...
        respx.post("https://api.openai.com/v1/chat/completions").mock(
            side_effect=[httpx.Response(
                200, json=mock_openai_chat_response) for _ in range(6)]
        )
        ...
        devset = [
            ...
            dspy.Example(question="Who wrote '1984'?",
                         answer="George Orwell").with_inputs("question"),
            dspy.Example(question="What year did World War II end?",
        ...
        eval_spans = [s for s in spans if "evaluation" in s.name.lower(
        ) or "metric" in s.name.lower()]
        ...
        assert len(predict_spans) >= len(
            devset), f"Expected at least {len(devset)} prediction spans, got {len(predict_spans)}"

        # Verify metric attributes
        metric_spans = [s for s in eval_spans if "metric" in s.attributes.get(
            "evaluation.type", "").lower()]
        if metric_spans:
            metric_span = metric_spans[0]
            assert metric_span.attributes.get(
                "metric.name") == "exact_match_metric"
            assert metric_span.attributes.get(
                "evaluation.samples") == len(devset)
    ...
    def test_error_handling(self, mock_openai_chat_response):
        ...
        assert len(
            error_spans) == 1, f"Expected 1 error span, got {len(error_spans)}"

        # Validate error attributes
        error_span = error_spans[0]
        assert "rate_limit" in error_span.attributes.get(
            "error.type", "").lower()
        assert error_span.attributes.get("http.status_code") == 429

        # Check for successful retry
        success_spans = [
            s for s in spans if s.status.is_ok and "Predict" in s.name]
        assert len(
            success_spans) >= 1, "Missing successful prediction after retry"
    ...
    async def test_streaming_responses(self, mock_openai_chat_response):
        ...
        if "streaming" in llm_span.attributes:
            ...
            stream_events = [e for e in getattr(
                llm_span, 'events', []) if "chunk" in e.name.lower()]
