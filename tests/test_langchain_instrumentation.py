...
class TestLangChainInstrumentation:
    ...
    def setup_teardown(self, in_memory_span_exporter):
        ...
        neatlogs.init(
            api_key="test-key",
            enable_otel=True,
            dry_run=True,  # IMPORTANT: Add dry_run=True to prevent server calls
    ...
    def test_lcel_basic_chain(self, mock_openai_chat_response):
        ...
        chain_spans = [s for s in spans if s.attributes.get(
            "openinference.span.kind") == "CHAIN"]
        llm_spans = [s for s in spans if s.attributes.get(
        ...
        runnable_spans = [s for s in spans if "runnable" in s.name.lower(
        ) or "sequence" in s.name.lower()]
        if runnable_spans:
            # Check for langchain framework attribute (might be in different attribute)
            for span in runnable_spans:
                framework = span.attributes.get(
                    "llm.framework") or span.attributes.get("framework") or ""
    ...
    def test_lcel_branching_chain(self, mock_openai_chat_response):
        ...
        branch = RunnableBranch(
            ...
            (check_simple,
             ChatPromptTemplate.from_template("Simple answer for: {input}") | model),
            ChatPromptTemplate.from_template(
                "Default answer for: {input}") | model
        )
    ...
    def test_legacy_llmchain(self, mock_openai_chat_response):
        ...
        llm = ChatOpenAI(api_key="fake", temperature=0.7,
                         model="gpt-3.5-turbo")
        ...
        if chain_spans:
            ...
            model_attr = chain_span.attributes.get(
                "llm.request.model") or chain_span.attributes.get("llm.model_name")
            if model_attr:
                assert "gpt" in model_attr.lower()

            # Check for input content
            input_attr = chain_span.attributes.get(
                "llm.input_messages") or chain_span.attributes.get("input.value")
            if input_attr:
    ...
    def test_legacy_sequential_chain(self, mock_openai_chat_response):
        ...
        prompt1 = PromptTemplate(
            ...
            template="Create a quiz question about {concept}."
        )
        ...
        llm_spans = [s for s in spans if s.attributes.get(
            "openinference.span.kind") == "LLM"]
        assert len(
            llm_spans) >= 2, f"Expected at least 2 LLM spans, got {len(llm_spans)}"
    ...
    def test_retrieval_qa_chromadb(self, mock_openai_chat_response, mock_embeddings_response):
        ...
        with patch('langchain_community.vectorstores.Chroma') as MockChroma:
            ...
            mock_docs = [
                Mock(page_content="LangChain is a framework for building LLM applications.",
                     metadata={"source": "test"}),
                Mock(page_content="OpenTelemetry is used for observability.",
                     metadata={"source": "test"})
            ]
        ...
        retriever_spans = [s for s in spans if "retriever" in s.name.lower() or
                           s.attributes.get("openinference.span.kind") in ["RETRIEVER", "TOOL", "CHAIN"]]
        if retriever_spans:
            retriever_span = retriever_spans[0]
            # Verify query was captured
            query_attr = retriever_span.attributes.get(
                "input.value") or retriever_span.attributes.get("retriever.query")
            if query_attr:
    ...
    def test_retrieval_with_hyde(self, mock_openai_chat_response, mock_embeddings_response):
        ...
        hyde_response["choices"][0]["message"]["content"] = "A hypothetical document about AI frameworks..."
        ...
        hyde_prompt = PromptTemplate(
            ...
            template="Write a paragraph that answers: {query}"
        )
    ...
    def test_agent_with_tools(self, mock_openai_chat_response):
        ...
        agent_executor = AgentExecutor(
            agent=agent, tools=[weather_tool], verbose=False)
        ...
        tool_spans = [s for s in spans if s.attributes.get(
            "openinference.span.kind") == "TOOL"]
        if tool_spans:
            tool_span = tool_spans[0]
            assert "get_weather" in tool_span.name.lower(
    ...
    def test_agent_with_multiple_tools(self, mock_openai_chat_response):
        ...
        result = executor.invoke({
            "input": "Calculate 2+2 then check London weather"
        })
        ...
        tool_spans = [s for s in spans if s.attributes.get(
            "openinference.span.kind") == "TOOL"]
        assert len(
            tool_spans) >= 2, f"Expected at least 2 tool spans, got {len(tool_spans)}"

        # Check for multiple LLM spans
        llm_spans = [s for s in spans if s.attributes.get(
            "openinference.span.kind") == "LLM"]
        assert len(
            llm_spans) >= 3, f"Expected at least 3 LLM spans, got {len(llm_spans)}"
    ...
    def test_pydantic_output_parser(self, mock_openai_chat_response):
        ...
        result = chain.invoke(
            {"input": "John Doe is 30 years old with email john@example.com"})

        spans = self.wait_for_spans(min_spans=3, timeout=3.0)

        # Look for parser spans (might be CHAIN kind)
        parser_spans = [s for s in spans if "parser" in s.name.lower() or
                        s.attributes.get("openinference.span.kind") in ["CHAIN", "PARSER"]]

        if parser_spans:
            parser_span = parser_spans[0]
            # Check for output parsing attributes
            output_attr = parser_span.attributes.get(
                "output.value") or parser_span.attributes.get("llm.output")
            if output_attr:
    ...
    def test_multi_output_parsing(self, mock_openai_chat_response):
        ...
        result = chain.invoke({
            "text": "Apple released great new products yesterday."
        })
        ...
        parser_spans = [
            s for s in spans if "json" in s.name.lower() or "output" in s.name.lower()]
        if parser_spans:
            parser_span = parser_spans[0]
    ...
    def test_lcel_streaming(self, mock_openai_chat_response):
        ...
        for span in llm_spans:
            streaming_attr = span.attributes.get(
                "llm.streaming") or span.attributes.get("streaming")
            if streaming_attr:
                assert streaming_attr is True or str(
                    streaming_attr).lower() == "true"
    ...
    def test_custom_runnable_instrumentation(self):
        ...
        if spans:
            ...
            custom_spans = [
                s for s in spans if "CustomRunnable" in s.name or "Runnable" in s.name]
            if custom_spans:
    ...
    def test_batch_processing(self, mock_openai_chat_response):
        ...
        for span in llm_spans:
            batch_size = span.attributes.get(
                "llm.batch_size") or span.attributes.get("batch.size")
            if batch_size:
